...
export default function AdminDashboardPage() {
  ...
  const stats = [
    { label: 'Total Orders',    value: fmt(ordersData?.total ?? 0),    icon: ShoppingBag, sub: 'all time' },
    { label: 'Products',        value: fmt(productsData?.total ?? 0),  icon: Package,     sub: 'in catalogue' },
    { label: 'Customers',       value: fmt(customersData?.total ?? 0), icon: Users,       sub: 'registered' },
    { label: 'Recent Revenue',  value: currency(totalRevenue),         icon: TrendingUp,  sub: 'latest 8 orders' },
  ];
    ...
    <RequireAdmin>
      <AdminLayout>
        <div className="max-w-6xl">
          <h1 className="font-serif text-2xl md:text-3xl mb-1">Dashboard</h1>
          <p className="font-sans text-sm text-muted-foreground mb-6 md:mb-8">
            ...
            {stats.map(({ label, value, icon: Icon, sub }) => (
              <div key={label} className="bg-card border border-border rounded-sm p-4 md:p-5">
                <div className="flex items-start justify-between mb-2 md:mb-3">
                  <p className="font-sans text-[10px] tracking-widest uppercase text-muted-foreground leading-tight">
                    {label}
                  </p>
                  <Icon className="size-4 text-muted-foreground/50 shrink-0" />
                </div>
                <p className="font-serif text-xl md:text-2xl mb-1">{value}</p>
          ...
          <div className="bg-card border border-border rounded-sm">
                ...
                <thead>
                  <tr className="border-b border-border">
                    {['Order', 'Status', 'Total', 'Date'].map((h) => (
                      <th key={h} className="px-6 py-3 text-left font-sans text-xs tracking-widest uppercase text-muted-foreground">
                        {h}
                      </th>
                    ))}
                  </tr>
            ...
            <div className="md:hidden divide-y divide-border">
              {recentOrders?.data?.length === 0 && (
                <p className="px-4 py-8 text-center text-muted-foreground font-sans text-sm">No orders yet</p>
              )}
              {recentOrders?.data?.map((order) => (
                <div key={order.id} className="px-4 py-3 flex items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <span className="font-sans text-xs font-medium">EGP {Number(order.total ?? 0).toLocaleString()}</span>
                  </div>
                </div>
              ))}
            </div>
